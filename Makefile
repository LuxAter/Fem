...
$(FEM): $(FEM_OBJS) FORCE
	...
	$(CXX) $(FEM_OBJS) $(ROOT)/$(LIBFEM.A) $(LINK) $(COMMON_INCLUDE) -o $(FEM)
...
build-libpng.a: pre-libpng.a
	...
	if [ ! -d "build/libpng.a" ]; then $(call print_run_cmd,make) && cd libpng && make install; fi
	$(call complete_target,libpng.a)

clean-libpng.a:
	$(call clean_target,libpng.a)
	if [ -e "libpng/Makefile" ]; then cd libpng && make clean && rm Makefile; fi
	if [ -d "build/libpng.a" ]; then rm build/libpng.a -r; fi
